namespace ChessDotNetCore
{
    ...
    public static class ZobristKey
    {
        ...
        private static readonly ulong[] PolyGlotRandoms = new ulong[781]
        {
        ...
        /// </summary>
        /// <param name="chessGame">      ChessGame</param>
        public static ulong ComputeBoardZobristKey(ChessGame chessGame)
        {
            ulong retVal = 0;
            int polyPiece;
            Piece? piece;
            ...
            {
                piece = chessGame.GetPieceAt(ChessUtilities.GetPositionAt(index));
                if (piece != null)
                {
                    polyPiece = piece.PieceValue;
                    retVal ^= PolyGlotRandoms[ZobPieceOffset + (64 * polyPiece + index)];
                }
            }
            if ((chessGame.CastlingType & CastlingType.WhiteCastleQueenSide) == CastlingType.WhiteCastleQueenSide)
                retVal ^= PolyGlotRandoms[ZobCastleOffset];
            if ((chessGame.CastlingType & CastlingType.WhiteCastleKingSide) == CastlingType.WhiteCastleKingSide)
                retVal ^= PolyGlotRandoms[ZobCastleOffset + 1];
            if ((chessGame.CastlingType & CastlingType.BlackCastleQueenSide) == CastlingType.BlackCastleQueenSide)
                retVal ^= PolyGlotRandoms[ZobCastleOffset + 2];
            if ((chessGame.CastlingType & CastlingType.BlackCastleKingSide) == CastlingType.BlackCastleKingSide)
                retVal ^= PolyGlotRandoms[ZobCastleOffset + 3];
            if (chessGame.EnPassant != null)
                retVal ^= PolyGlotRandoms[ZobEnPassantOffset + (int)chessGame.EnPassant.Line];
            if (chessGame.CurrentPlayer == Player.White)
                retVal ^= PolyGlotRandoms[ZobTurnOffset];
            return retVal;
        }
        #endregion
    } // Class ZobristKey
